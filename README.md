...
> Current status: **0.6.28-dev** — the host boot chain, exact-firmware authorization, profile-driven kernel/device-tree evidence and kernel reproducibility contracts are in place. The avicii kernel baseline's Android Clang dependency is source-locked to immutable AOSP Gitiles commit/tree/blob identities for `clang-r416183b`; the exact kernel checkout's `build.config.common` is bound to that lock, and schema-v1 toolchain candidate evidence now requires the immutable source evidence, checkout/compiler-selection evidence and concrete materialized `clang` SHA-256/banner evidence to agree on the same kernel plan. First-boot candidate schema v7 carries those compiler inputs together with exact firmware, boot, reproducible kernel, DTB/DTBO and rootfs evidence. The latest completed ARM64 rootfs double-build failed strict byte-for-byte acceptance; the next diagnostics-enabled real double-build is still running, so `rootfs_reproducible_artifact` remains false. This remains **host-side provenance and preflight only**: no final kernel/DTB/DTBO candidate or Kali userspace has been proven on a physical AC2003. No public Beta is allowed until the physical device passes `BETA_RELEASE_GATE.md`.
...
- First-boot candidate manifest schema v7 carries Fastboot baseline/exact firmware identity, temporary-boot authorization, boot-plan digest, reproducible kernel evidence, exact compiler source/selection/materialized-byte evidence, exact structural DTB/DTBO evidence and rootfs/package evidence in one canonical host-side contract.
