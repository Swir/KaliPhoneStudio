...
The audit exists to stop source, documentation, and test contracts from drifting independently. It does **not** connect to a phone, import an execution path, resolve a raw `/dev/...` target, mount storage, authorize writes, perform a trial, verify hardware, or grant Beta credit.

## Audited contracts

The checker binds the currently reviewed policy identifiers to their source, documentation, and focused tests:
...
| Local rootfs preflight | `rootfs-handoff-local-rootfs-preflight-v1` | The exact local Kali rootfs bytes are re-hashed while all physical/device/write permissions remain false. |
...
The machine-readable report itself also hard-codes all physical interaction, write, hardware, and Beta claims to `false`.
...
The audit tests intentionally tamper with copied contracts to prove that policy drift, removal of a Beta fail-closed flag, and documentation safety-marker drift are rejected.

## CI contract

`.github/workflows/rootfs-trial-chain-audit.yml` runs the deterministic audit and the focused trial-chain test set on Python 3.11 and 3.14 when the audited source, documentation, tests, audit tool, or workflow changes.
