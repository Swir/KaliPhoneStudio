...
- reviewed executable SHA-256: `e84d038e803b07503aa843aca5a82b0441877afc6cd5025247dd31d6046d02c3`.

Exact-head A/B reproducibility run `35643537783` on **2026-09-21** used the locked MSYS2 mingw64 package set, including GCC `16.2.0-4` and the r420 CRT/headers/winpthreads revisions. It built the exact Windows executable twice and proved byte-for-byte equality at the digest above. The workflow then recursively staged the non-system PE runtime closure and passed the clean-PATH smoke before that digest was promoted into the lock. For this exact r420 build the recursive closure contained zero non-system runtime dependencies, so no MinGW DLL copies were needed. This supersedes the prior reviewed digest `9a4848ff93b28a9c2f9dbf52b11e09184242ff192032187d70079df5dfb9a616`, which was produced under the previous r409 CRT/headers/winpthreads revision.

The packaging policy stays conservative even when an exact build happens to need no copied DLLs: `-extldflags=-static` is not treated as proof that every possible MinGW runtime import is absent. KaliPhoneStudio therefore does not label the binary fully static. `stage_windows_extractor_runtime.ps1` recursively inspects the executable and every copied DLL with MinGW `objdump`, treats Windows system/API-set imports as operating-system dependencies, and copies each remaining MinGW dependency next to `payload-dumper-go.exe`. Every copied file is re-hashed, the complete closure is recorded in `operator-extractor-runtime.json`, and unresolved non-system imports fail the build.
