...
def test_command_line_archive_policy_is_exported_to_make_recipe(tmp_path: Path) -> None:
    make = shutil.which("make")
    if make is None:
        pytest.skip("GNU make is not installed on this host")

    flags = dict(_plan().make_flags)
    ...
    result = subprocess.run(
        [
            ...
            "-C",
            str(tmp_path),
            f"TAR_OPTIONS={flags['TAR_OPTIONS']}",
            f"XZ_OPT={flags['XZ_OPT']}",
        ],
        check=True,
    ...
    assert result.stdout.splitlines() == [flags["TAR_OPTIONS"], flags["XZ_OPT"]]
