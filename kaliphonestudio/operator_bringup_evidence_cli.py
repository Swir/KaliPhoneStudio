"""Offline operator CLI for late-stage physical bring-up evidence.

This module intentionally performs no phone I/O and imports no ADB/Fastboot or
subprocess execution path.  It only cross-binds exact evidence that was already
captured through separately gated physical workflows.  Successful commands are
manual audit inputs only: they cannot select a storage target, authorize a
persistent write, prove hardware support or authorize a Beta release.
"""
...
def _add_out(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--out", type=Path, required=True, help="New output path; existing files are never overwritten.")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Offline exact-file bring-up audit workspace. These commands do not connect to a phone, "
    ...
    session = sub.add_parser(
        "bind-bringup-session",
        help="Cross-bind one exact physical candidate/rescue/storage chain into a non-release audit session.",
        description=(
            "Offline-only: binds already recorded candidate, rescue, functional-probe and reviewed storage evidence. "
            "Optional Kali early-userspace evidence is accepted only when it matches the same exact chain."
        ),
    )
    ...
    dossier = sub.add_parser(
        "build-bringup-dossier",
        help="Freeze one bring-up session and every exact source file into an immutable audit dossier.",
        description=(
            "Offline-only: verifies the exact evidence/raw file bytes named by one session before writing a dossier. "
    ...
    verify = sub.add_parser(
        ...
        description=(
            "Offline-only: repeat --file ROLE=PATH exactly once for every role recorded by the dossier. "
            "Successful reverification is integrity evidence only, never hardware/Beta credit."
        ),
    )
    verify.add_argument("--dossier", type=Path, required=True)
    verify.add_argument("--file", action="append", type=_role_file, required=True, metavar="ROLE=PATH")
    _add_out(verify)

    review_template = sub.add_parser(
        "prepare-bringup-dossier-review",
        help="Create rejected-by-default review record and notes templates for one exact dossier.",
    )
    ...
    review = sub.add_parser(
        "bind-bringup-dossier-review",
        help="Bind exact dossier, independent reverification, manual review record and notes.",
        description=(
            "Offline-only: accepted review permits only a later separate strategy review. "
            "It cannot choose a target, authorize a write or grant hardware/Beta credit."
        ),
    )
    review.add_argument("--dossier", type=Path, required=True)
    review.add_argument("--verification", type=Path, required=True)
    review.add_argument("--review-record", type=Path, required=True)
    review.add_argument("--review-notes", type=Path, required=True)
    _add_out(review)

    audit = sub.add_parser(
        "build-release-gate-audit",
        help="Cross-bind accepted bring-up and functional campaigns into one manual release-gate audit packet.",
        description=(
            "Offline-only: requires the exact dossier/reverification/review plus exact functional result bundle, "
            "plan and accepted plan review. The output explicitly keeps the physical gate incomplete and Beta unauthorized."
        ),
    )
...
def _safe_result(command: str, destination: Path | str, digest: str | None = None, **extra: object) -> dict[str, object]:
...
def _emit(result: dict[str, object], *, as_json: bool) -> None:
    ...
    hidden = {
        "command", "path", "sha256", "physical_interaction_performed", "external_device_command_executed",
        "storage_target_selected", "persistent_write_authorized", "phone_storage_written", "hardware_verified",
...
def _run(args: argparse.Namespace) -> dict[str, object]:
    ...
    if command == "bind-bringup-session":
        ...
        evidence = bind_physical_bringup_session(
            gate, observation, diagnostics, functional, discovery, review, early_userspace=early
        )
        digest = write_physical_bringup_session_evidence(evidence, args.out)
        return _safe_result(
            command, args.out, digest,
            profile_id=evidence.profile_id,
            device_serial=evidence.device_serial,
            rescue_chain_complete=evidence.rescue_chain_complete,
            storage_review_recorded=evidence.storage_review_recorded,
            storage_review_accepted_for_strategy_design=evidence.storage_review_accepted_for_strategy_design,
    ...
    if command == "build-bringup-dossier":
        ...
        return _safe_result(
            ...
            exact_files_verified=dossier.supplied_file_count,
    ...
    if command == "prepare-bringup-dossier-review":
        if not _SAFE_REVIEWER_RE.fullmatch(args.reviewer):
            raise ValueError("reviewer must be a safe identifier using letters, numbers, . _ @ + or -")
        dossier = load_physical_bringup_dossier_evidence(args.dossier)
        ...
        record_digest = _write_new(args.record_out, record.canonical_json())
        try:
            notes_digest = _write_new(args.notes_out, notes)
        except Exception:
            try:
                args.record_out.unlink()
            except OSError:
                pass
            raise
        return _safe_result(
            command, args.record_out, record_digest,
    ...
    if command == "bind-bringup-dossier-review":
        ...
        return _safe_result(
            ...
            dossier_review_completed=evidence.dossier_review_completed,
    ...
    raise ValueError(f"unsupported bring-up evidence command: {command}")
