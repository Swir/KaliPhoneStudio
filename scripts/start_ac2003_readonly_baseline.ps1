...
function Get-ReviewedAdbVersionLine([string]$VersionOutput, [string]$ExpectedVersion) {
    ...
    $VersionLines = @(
        $VersionOutput -split "`r?`n" |
            ...
            Where-Object { -not [string]::IsNullOrWhiteSpace($_) -and $_ -match '^Version\s+' }
    )
...
exit 0,
    ...
    if (-not $Match.Success) {
        throw "ADB --version Version line has an unsupported format: $VersionLine"
    }
    $ObservedVersion = $Match.Groups["base"].Value
    if ($ObservedVersion -ne $ExpectedVersion) {
        throw "ADB version drift: policy requires $ExpectedVersion; observed base version $ObservedVersion; reported: $VersionOutput"
    }
    return $VersionLine
}
