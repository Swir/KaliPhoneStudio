...
- [x] The same source/frozen Windows `evidence` surface now covers physical bring-up session binding, exact dossier build, independent post-transfer reverification, rejected-by-default dossier review/binding and cross-campaign release-gate audit. Dedicated Windows CI proves representative missing/detached inputs fail closed; [`docs/PHYSICAL_BRINGUP_AUDIT_CLI.md`](docs/PHYSICAL_BRINGUP_AUDIT_CLI.md) documents the exact-file operator flow.
- [ ] Add GUI affordances for the late exact-file review/audit stages only if the interface can keep every filename, reviewer decision, no-write boundary and physical/Beta non-promotion state explicit.
...
The highest-value next step is **real physical AC2003 evidence**, not another guessed block-device target. With the physical phone available, use the guarded profile-token capture CLI for the exact baseline, then continue through the shared stock baseline → candidate-gate → temporary-boot offer → explicitly opted-in one-shot temporary rescue boot chain. The offline `evidence` workspace can then carry the same real campaign through rescue diagnostics → survey/review → functional plan/review → schema-v2 observations/result reviews → exact-plan summary/result bundle → storage review → bring-up session → exact dossier/reverification/review → cross-campaign release-gate audit, while keeping exact filenames and manual-review boundaries visible. Only then design and review the reversible rootfs handoff target for that exact device state.
