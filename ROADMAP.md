...
- [x] Shared source/frozen Windows `evidence` namespace now also covers physical bring-up session binding, exact dossier construction/reverification, fail-closed dossier review templates/binding, cross-campaign release-gate audit, reversible rootfs-strategy review, exact logical target-binding review and distinct-capture fresh target revalidation. These late stages remain offline, create-only/exact-file bound, and cannot bind raw storage paths, mount, execute a rootfs trial, authorize writes, claim hardware support or authorize Beta.
