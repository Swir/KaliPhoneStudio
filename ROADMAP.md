...
- [ ] Consolidate the remaining physical bring-up session/dossier/review and cross-campaign release-gate audit stages into a polished Windows GUI/CLI while preserving explicit confirmation, exact-file review and no-write boundaries.
...
The highest-value next step is **real physical AC2003 evidence**, not another guessed block-device target. With the physical phone available, use the guarded profile-token capture CLI for the exact baseline, then continue through the now-shared stock baseline → candidate-gate → temporary-boot offer → explicitly opted-in one-shot temporary rescue boot chain. The offline `evidence` workspace can then bind rescue diagnostics → survey/review → functional plan/review → schema-v2 observations/result reviews → exact-plan summary → exact functional result bundle, all while keeping exact filenames and manual-review boundaries visible. Continue through the separate session/dossier chain → cross-campaign release-gate audit. Only then design and review the reversible rootfs handoff target for that exact device state.
