...
- [x] Shared offline `evidence` CLI now consolidates rescue-diagnostics binding, hardware-survey binding/review, storage-review preparation/binding and functional test-plan/review preparation/binding; a dedicated Windows workflow freezes that same CLI and checks representative fail-closed refusals without granting physical/Beta credit.
- [ ] Consolidate the later physical observation/result/bundle/session/dossier/audit stages into a polished Windows GUI/CLI while preserving explicit confirmation, exact-file review and no-write boundaries.
...
The highest-value next step is **real physical AC2003 evidence**, not another guessed block-device target. With the physical phone available, use the guarded profile-token capture CLI for the exact baseline, then continue through the now-shared stock baseline → candidate-gate → temporary-boot offer → explicitly opted-in one-shot temporary rescue boot chain. The offline `evidence` workspace can then bind rescue diagnostics → survey/review → functional plan/review and storage review without hiding exact filenames or manual review boundaries. Continue through schema-v2 observations/reviews → exact functional result bundle → session/dossier chain → cross-campaign release-gate audit. Only then design and review the reversible rootfs handoff target for that exact device state.
