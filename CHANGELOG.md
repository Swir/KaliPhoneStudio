...
- Real AC2003 phase-2 host fix: the physical wizard, read-only baseline launcher and host preflight now chain child PowerShell scripts through the exact current `$PSHOME\pwsh.exe` instead of resolving bare `pwsh` from PATH. Each layer self-checks that host, preventing a different/incomplete PowerShell installation from losing built-in Core modules after the ADB -> bootloader transition.
- Real AC2003 Fastboot enumeration: a 60-second Fastboot discovery timeout now creates a host-only Windows Fastboot USB diagnostic with PnP/signed-driver status and a bounded diagnosis (`windows-driver-not-installed`, `windows-pnp-device-problem`, `usb-device-present-but-fastboot-not-bound` or `no-usb-pnp-device-seen`). It never installs/changes a driver and grants no hardware/Beta credit.
- Real AC2003 mode transition: FIRST TEST now offers either manual phone controls or an explicitly confirmed ADB reboot-to-bootloader transition. The ADB path verifies the unchanged reviewed executable, exact captured ADB serial and authorized `get-state`, requires the literal token `REBOOT-BOOTLOADER AC2003`, records a create-only mode-transition evidence file, waits for exactly one Fastboot target and still authorizes no flash/erase/slot change/persistent write or Beta credit.
- Real AC2003 bring-up: the read-only ADB identity phase now has a bounded 120-second authorization wait. `unauthorized`, `offline` and no-device states get explicit operator guidance and automatic 2-second retries; exactly one authorized device continues automatically, multiple targets fail closed, and CI simulates unauthorized -> authorized transition end-to-end.
- PowerShell parser guard now covers the packaged read-only baseline before Windows packaging, and both Windows physical-candidate workflows reproduce the real CRLF-separated ADB 37.0.1 build-suffix output before artifacts are uploaded.
- Fixed real-device FIRST TEST rejecting the reviewed Android Platform-Tools 37.0.1 ADB because native Windows `adb --version` uses a CRLF-separated `Version 37.0.1-<build>` line. The parser now trims and isolates exactly one Version line, extracts and compares the exact base version `37.0.1`, permits only a non-whitespace build suffix, and keeps all other versions fail-closed; Windows candidate CI reproduces the real multiline format.
...
- Accepted target binding now requires one non-removable block identity for the reviewed role, an expected observed filesystem, a physically observed `unlocked` or `unencrypted` state and enough free space for the reviewed rootfs requirement. Forbidden roles, report drift, removable targets, locked encryption, undersized capacity and record drift fail closed.
- The new evidence deliberately carries no raw `/dev` path or mount target and always keeps `trial_execution_allowed=false`, `write_authorized=false`, `phone_storage_written=false`, `hardware_verified=false`, `beta_release_authorized=false` and `beta_gate_credit=false`; a later fresh-device/manual trial gate remains mandatory.
- Added rejected-by-default target-binding review templates, canonical exact-file load/write validation, standalone `scripts/build_rootfs_handoff_target_binding.py`, shared `python main.py evidence ...` commands, operator documentation and focused regression tests.
- Added dedicated Python 3.11/3.14 `rootfs-handoff-target-binding` CI and extended shared evidence-workspace tests so target-binding commands cannot silently disappear or gain a device-execution import.
- Synchronized README, ROADMAP, BUILD_STATUS and `BETA_RELEASE_GATE.md` with the new host-side safety milestone while preserving the existing SWIR SVG-only progress presentation.
- Project completion remains **58%** and Beta remains **BLOCKED** because no real AC2003 storage report/accepted strategy/logical target binding, temporary boot, physical subsystem validation or exercised recovery/rollback has passed the mandatory gate.
